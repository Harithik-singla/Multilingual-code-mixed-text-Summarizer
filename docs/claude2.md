...
- Status: **NOT YET RUN** (code is ready)
...
- Status: **NOT IMPLEMENTED** (pipeline stub only)
...
| `src/components/inference/predictor.py` | End-to-end inference pipeline |
| `src/components/inference/language_detector.py` | Auto-detect dominant language |
| `src/pipelines/train_phase2.py` | Phase 2 training pipeline |
| `src/pipelines/train_phase3.py` | Phase 3 training pipeline |
...
# Step 4: Train Phase 1 (NEXT TO DO)
python -m src.pipelines.train_phase1
# To quick-test with small data, edit MAX_SAMPLES_PER_LANGUAGE = 200 in the file
```
...
2. **GPU setup:** System has RTX 3050 (4GB VRAM), CUDA 13.2. Need PyTorch CUDA build installed (`pip install torch --index-url https://download.pytorch.org/whl/cu124`). CPU-only build was previously installed.

3. **4GB VRAM:** May need batch_size=1 or gradient checkpointing for full dataset training on 4GB GPU.
...
```
1. Install PyTorch CUDA and run Phase 1 training
2. train_phase2.py    ← Phase 2 pipeline
3. train_phase3.py    ← Phase 3 pipeline  
4. predictor.py       ← Inference
5. language_detector.py
6. inference_pipeline.py
7. app.py             ← Web demo
8. Tests
```
